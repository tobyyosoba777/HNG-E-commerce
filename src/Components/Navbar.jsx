...
const Navbar = () => {
  ...
  return (
    <nav className="sticky z-50 flex items-center justify-between px-10 mx-auto mt-2 text-gray-700 bg-white w-10-full md:border md:border-gray-400 md:rounded-full top-2 max-w-md:rounded-none max-w-md:border-none sm:w-4/5 md:w-4/5 lg:w-3/4 xl:w-11/12">
      <img alt="Logo-icon" src={LightLogo} className="ml-2" width={80} height={80} />
      
      <ul className="items-center justify-center hidden mr-5 space-x-12 lg:flex">
        {NavItems.map((item) => (
          <li key={item.id}>
            <NavLink to={item.link} className={({ isActive }) => isActive ? 'font-bold' : 'text-gray-700'}>
          ...
          <ul>
            {NavItems.map((item) => (
              <li key={item.id} className="py-1 pb-12 text-center">
                <NavLink to={item.link} className="flex items-center justify-center gap-3">
                  {item.icon}
                  {item.text}
                </NavLink>
              </li>
